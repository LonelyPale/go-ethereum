...
)](https://pkg.go.dev/github.com/lifefile/go-lifefile?tab=doc)
[![Go Report Card](https://goreportcard.com/badge/github.com/lifefile/go-lifefile)](https://goreportcard.com/report/github.com/lifefile/go-lifefile)
[![Travis](https://travis-ci.com/lifefile/go-lifefile.svg?branch=master)](https://travis-ci.com/lifefile/go-lifefile)
...
The go-lifefile project comes with several wrappers/executables found in the `cmd`
directory.
...
|   `rlpdump`   | Developer utility tool to convert binary RLP ([Recursive Length Prefix](https://eth.wiki/en/fundamentals/rlp)) dumps (data encoding used by the LifeFile protocol both network as well as consensus wise) to user-friendlier hierarchical representation (e.g. `rlpdump --hex CE0183FFFFFFC4C304050583616263`).                                                                                                                                                                                                                                 |
|   `puppeth`   | a CLI wizard that aids in creating a new LifeFile network.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
### Full node on the main LifeFile network

By far the most common scenario is people wanting to simply interact with the LifeFile
...
Transitioning towards developers, if you'd like to play around with creating LifeFile
 ...
 * Instead of connecting the main LifeFile network, the client will connect to the Görli
   test network, which uses different P2P bootnodes, different network IDs and genesis
   states.
 * Instead of using the default data directory (`~/.lifefile` on Linux for example), `geth`
   will nest itself one level deeper into a `goerli` subfolder (`~/.lifefile/goerli` on
...
Go LifeFile also supports connecting to the older proof-of-authority based test network
...
One of the quickest ways to get LifeFile up and running on your machine is by using
Docker:

```shell
docker run -d --name lifefile-node -v /Users/alice/lifefile:/root \
...
LifeFile nodes with exposed APIs! Further, all browser tabs can access locally
...
Mining on the public LifeFile network is a complex task as it's only feasible using GPUs,
requiring an OpenCL or CUDA enabled `ethminer` instance. For information on such a
setup, please consult the [EtherMining subreddit](https://www.reddit.com/r/EtherMining/)
and the [ethminer](https://github.com/lifefile-mining/ethminer) repository.
...
If you'd like to contribute to go-lifefile, please fork, fix, commit and send a pull request
for the maintainers to review and merge into the main code base. If you wish to submit
more complex changes though, please check up with the core devs first on [our gitter channel](https://gitter.im/lifefile/go-lifefile)
...
The go-lifefile library (i.e. all code outside of the `cmd` directory) is licensed under the
[GNU Lesser General Public License v3.0](https://www.gnu.org/licenses/lgpl-3.0.en.html),
also included in our repository in the `COPYING.LESSER` file.

The go-lifefile binaries (i.e. all code inside of the `cmd` directory) is licensed under the
