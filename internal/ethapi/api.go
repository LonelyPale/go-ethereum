...
// Sign calculates an LifeFile ECDSA signature for:
// keccack256("\x19LifeFile Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/lifefile/life-file/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19LifeFile Signed Message:\n"${message length}${message})
...
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid LifeFile signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the LifeFile blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new LifeFile blockchain API.
func NewPublicBlockChainAPI(b Backend) *PublicBlockChainAPI {
	return &PublicBlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the currentlifefile.chain config.
...
//
// https://github.com/lifefile/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of LifeFile APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of LifeFile APIs exposed over the private
// debugging endpoint.
...
// Version returns the currentlifefile.protocol version.
