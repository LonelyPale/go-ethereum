...
import (
	...
	"github.com/lifefile/life-file/crypto/signify"
	"github.com/lifefile/life-file/internal/build"
...
var (
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert LifeFile contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
...
func doInstall(cmdline []string) {
	...
	if !strings.Contains(runtime.Version(), "devel") {
		...
		if minor < 13 {
			...
			log.Println("life-file requires at least Go version 1.13 and cannot")
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "lifefile/lifefile")`)
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, pkg := range debPackages {
		for distro, goboot := range debDistroGoBoots {
			// Prepare the debian package with the life-file sources.
...
type debPackage struct {
	Name        string          // the name of the Debian package to produce, e.g. "lifefile"
...
type debMetadata struct {
	...
	// life-file version being built. Note that this
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.lifefile", "-v", "github.com/lifefile/life-file/mobile"))
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/lifefile/life-file/mobile")
