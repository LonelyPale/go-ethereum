...
// information related to the LifeFile protocol such als blocks, transactions and logs.
...
func (api *PublicFilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(lifefile.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as lifefile.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria lifefile.FilterQuery
...
func (api *PublicFilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(lifefile.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
