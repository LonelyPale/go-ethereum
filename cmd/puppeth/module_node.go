...
// nodeDockerfile is the Dockerfile required to run an LifeFile node.
var nodeDockerfile = `
FROM lifefile/client-go:latest
...
RUN \
  echo 'geth --cache 512 init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.lifefile/keystore/ && cp /signer.json /root/.lifefile/keystore/' >> geth.sh && \{{end}}
...
// an LifeFile node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    ports:
      ...
      - "{{.Port}}:{{.Port}}/udp"
    volumes:
      - {{.Datadir}}:/root/.lifefile{{if .Ethashdir}}
...
// deployNode deploys a new LifeFile node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.lifefile"],
