...
// geth is the official command-line client for LifeFile.
package main
...
import (
	...
	"github.com/lifefile/life-file/console/prompt"
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the life-file command line interface")
...
func init() {
	...
	app.Copyright = "Copyright 2013-2021 The life-file Authors"
...
func prepare(ctx *cli.Context) {
	...
	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Geth on LifeFile mainnet...")
	}
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend) {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full LifeFile node is running
		if ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		ethBackend, ok := backend.(*eth.EthAPIBackend)
		if !ok {
			utils.Fatalf("LifeFile service not running: %v", err)
		}
