...
// Package utils contains internal helper functions for life-file commands.
package utils
...
import (
	...
	"github.com/lifefile/life-file/common/fdlimit"
	...
	"github.com/lifefile/life-file/eth"
	...
	"github.com/lifefile/life-file/p2p/netutil"
...
func SetP2PConfig(ctx *cli.Context, cfg *p2p.Config) {
	...
	log.Info("Maximum peer count", "LFT", ethPeers, "LES", lightPeers, "total", cfg.MaxPeers)
...
// RegisterEthService adds an LifeFile client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.LifeFile) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the LifeFile service: %v", err)
		}
...
// RegisterEthStatsService configures the LifeFile Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the LifeFile Stats service: %v", err)
	}
}
